...
- **Coding-agent integrations** for Codex, Claude Code, OpenCode, Grok, and Antigravity, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside Codex, Claude Code, OpenCode, Grok, or Antigravity, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
openwiki integrations install grok
openwiki integrations install antigravity
```

The supported targets are **Codex**, **Claude Code**, **OpenCode**, **Grok**, and **Antigravity**. All install at user level by default, so one installation works from any Git repository. Project paths are resolved to their Git repository root. Antigravity is user-scope only, because its CLI reads MCP servers from one global `~/.gemini/config/mcp_config.json`; `--project` is rejected for that host. User-level OpenCode integrations live under `~/.config/opencode`, OpenCode's global configuration directory on every supported platform. Grok integrations install into `~/.grok` at user level and `.grok/` at project level, both reading `config.toml`. Restart the coding agent after installation, open the repository, and ask:
...
The integration exposes the same five operations as native generation: `openwiki_begin`, `openwiki_submit_plan`, `openwiki_next_page`, `openwiki_submit_page`, and `openwiki_finish`. The host agent submits the complete intended Claim set with each page; OpenWiki internally preserves, updates, creates, or retracts Claims and refuses to finish until the final state is durable.
...
openwiki integrations install <codex|claude|opencode|grok|antigravity> [--project [path]]
openwiki integrations uninstall <codex|claude|opencode|grok|antigravity> [--project [path]]
