...
function createCredentialDiagnostic(
  ...
  return {
    ...
    warnings:
      ...
      key === OPENWIKI_PLANNER_MODEL_ID_ENV_KEY ||
      key === OPENWIKI_PAGE_MODEL_ID_ENV_KEY ||
      key === OPENWIKI_SPECIALIST_MODEL_ID_ENV_KEY
...
function isNonSecretDiagnosticKey(key: string): boolean {
  return (
    ...
    key === OPENWIKI_PAGE_MODEL_ID_ENV_KEY ||
    key === OPENWIKI_SPECIALIST_MODEL_ID_ENV_KEY ||
    key === OPENWIKI_SPECIALIST_PATH_PREFIXES_ENV_KEY ||
    key === OPENWIKI_PROVIDER_ENV_KEY ||
    key === OPENWIKI_MAX_OUTPUT_TOKENS_ENV_KEY ||
    key === OPENWIKI_BEDROCK_MAX_TOKENS_ENV_KEY ||
    key === OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY ||
